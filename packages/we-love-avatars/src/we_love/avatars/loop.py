...
class Loop:
    ...
    def _calculate_points(
        ...
        if self.config.loop_type == LoopType.CIRCLE:
            ...
            y = center_y + scale * np.sin(
                self.config.freq_y * t + self.config.phase + anim_phase
            )
            ...
            d = (R + r) / 2
            x = center_x + (R + r) * np.cos(t + anim_phase) - d * np.cos(
                ((R + r) / r) * (t + anim_phase)
            )
            y = center_y + (R + r) * np.sin(t + anim_phase) - d * np.sin(
                ((R + r) / r) * (t + anim_phase)
            )
